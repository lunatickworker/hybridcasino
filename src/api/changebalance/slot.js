export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('[Vercel Proxy] /changebalance/slot request:', {
      method: req.method,
      body: bodyString
    });

    // Supabase Edge Function으로 프록시 (Authorization 헤더 추가)
    const response = await fetch(`${SUPABASE_URL}/functions/v1/server/changebalance/slot`, {
      method: 'POST',
      headers: {
    ...
    console.log('[Vercel Proxy] /changebalance/slot response:', {
    ...
    console.error('[Vercel Proxy] /changebalance/slot error:', error);
