  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // 500 에러는 정보로만 출력 (정상 상황)
        if (response.status === 500) {
          console.log(`ℹ️ Proxy 500 에러: ${errorText}`);
        } else {
          console.log(`⚠️ Proxy 서버 응답 (${response.status}):`, errorText);
        }
...
export async function getUserBalance(opcode: string, username: string): Promise<{ success: boolean; balance?: number; error?: string }> {
  try {
    ...
    if (configError || !apiConfig) {
      console.error('❌ [보유금 조회] api_configs 조회 실패:', { 
        opcode, 
        error: configError,
...
// ⭐ Optimistic Update 적용: api_configs, users balance 먼저 증감
export async function withdrawBalance(opcode: string, username: string, token: string, amount: number, secretKey: string) {
  ...
  if (configError || !apiConfig) {
    console.error('❌ [Withdraw Optimistic] api_configs 조회 실패:', configError);
    // 에러여도 기존 로직은 계속 진행 (하위 호환성)
  }
  ...
  if (apiConfig) {
    ...
    if (updateConfigError) {
      ...
      console.log(`✅ [Withdraw Optimistic] api_configs 증가 완료: ${previousInvestBalance} → ${previousInvestBalance + amount}`);
      ...
      if (!userError && userData) {
        ...
        console.log(`🔄 [Withdraw Optimistic] users balance 업데이트 시작: ${amount}원으로 설정`);
        ...
        if (updateUserError) {
          console.error('❌ [Withdraw Optimistic] users 업데이트 실패:', updateUserError);
        } else {
          console.log(`✅ [Withdraw Optimistic] users 업데이트 완료: ${previousUserBalance} → ${amount}`);
        }
      }
    }
  }
  
  // ⭐ 4. 외부 API 출금 호출 (기존 로직)
  ...
  if (result.error && optimisticUpdateApplied && apiConfig) {
    console.error('❌ [Withdraw API 실패] 롤백 시작');
    ...
    if (userId) {
      ...
      console.log(`🔄 [Rollback] users 복구: ${amount} → ${previousUserBalance}`);
    }
  }
...
// ✅ 파트너의 API 설정 조회 (api_configs 테이블 사용)
