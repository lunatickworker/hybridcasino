...
import { setManualSyncActive } from '../../lib/oroplayApi';
...
export function OroPlayAutoSync() {
    ...
    const checkSession = async () => {
      try {
        ...
        if (session?.access_token) {
          ...
          console.log('✅ [OroPlay Auto Sync] 세션 토큰 확인 완료');
        }
        // 세션이 없어도 경고하지 않음 (이 앱은 partners 테이블로 인증)
      } catch (error) {
        console.error('❌ [OroPlay Auto Sync] 세션 확인 실패:', error);
      }
    };
  ...
  useEffect(() => {
    ...
    const syncBets = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          console.error('❌ [OroPlay Auto Sync] 동기화 에러:', response.status, errorText);
          setStats(prev => ({
            ...prev,
    ...
    // 1초마다 반복
    const interval = setInterval(syncBets, 1000);
  ...
  const handleManualSync = async () => {
    ...
    try {
      ...
      console.log('📍 요청 URL:', `${SERVER_URL}/sync/oroplay-bets`);
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('❌ [OroPlay Manual Sync] 동기화 에러:', response.status, errorText);
        
        if (response.status === 404) {
          toast.error('Edge Function 라우트를 찾을 수 없습니다. /sync/oroplay-bets 엔드포인트가 배포되었는지 확인해주세요.');
        } else {
