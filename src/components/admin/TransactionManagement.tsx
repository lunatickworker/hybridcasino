...
export function TransactionManagement({ user }: TransactionManagementProps) {
  ...
  const loadData = async (isInitial = false) => {
    ...
    try {
      ...
      if (user.level > 1) {
        // ✅ 조직격리: 현재 사용자와 관련된 거래만 조회
        // - from_partner_id = 현재 사용자 (내가 보낸 거래)
        // - to_partner_id = 현재 사용자 (내가 받은 거래)
        const currentPartnerId = user.id;
        partnerTransactionQuery = partnerTransactionQuery.or(`from_partner_id.eq.${currentPartnerId},to_partner_id.eq.${currentPartnerId}`);
      }
  ...
  const getColumns = (showActions = false) => [
    ...
    },
    {
      ...
      cell: (row: any) => {
        // 파트너 거래인 경우
        if (row.is_partner_transaction && row.from_partner_nickname) {
          return (
    ...
    },
    {
      ...
      cell: (row: any) => {
        // 파트너 거래인 경우
        if (row.is_partner_transaction && row.to_partner_nickname) {
          return (
