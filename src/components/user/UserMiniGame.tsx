        ...
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {Array.from({ length: 10 }).map((_, i) => (
              <Card key={i} className="luxury-card animate-pulse border-green-600/20">
                <div className="aspect-square bg-gradient-to-br from-slate-700 to-slate-800 rounded-xl" />
              </Card>
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {filteredGames.map((game) => (
              <Card 
                key={game.game_id} 
                className={`luxury-card group cursor-pointer border-2 border-green-600/20 hover:border-green-500/60 transition-all game-card-hover overflow-hidden ${
                ...
                <div className="aspect-square relative overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all duration-300 flex items-center justify-center">
                    {launchingGameId === game.game_id ? (
                      ...
                      <Button 
                        size="sm" 
                        className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white font-bold"
                      >
