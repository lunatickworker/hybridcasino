...
export function UserSlot({ user, onRouteChange }: UserSlotProps) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('partner_game_access_slot_changes')
      .on(
        'postgres_changes',
        {
        ...
        (payload) => {
          console.log('🔄 [UserSlot] partner_game_access 변경 감지:', payload);
          
          // 현재 선택된 제공사 게임을 다시 로드
          if (selectedProvider && selectedProvider !== "all") {
            console.log('🔄 [UserSlot] 게임 목록 자동 갱신 (provider:', selectedProvider, ')');
            loadSlotGames(parseInt(selectedProvider));
          } else if (selectedProvider === "all") {
            console.log('🔄 [UserSlot] 전체 게임 목록 자동 갱신');
            loadAllSlotGames();
          }
        }
      )
    ...
    try {
      ...
      if (activeSession?.isActive && 
        ...
        if (!gameWindow) {
          ...
          // ⭐ 게임 창 닫힐 때 heartbeat도 정리
          const originalCloseHandler = handleGameWindowClose;
