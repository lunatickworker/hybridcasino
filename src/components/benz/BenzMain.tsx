...
export function BenzMain({ user, onRouteChange }: BenzMainProps) {
  ...
  return (
    <div className="space-y-8" style={{ fontFamily: '\"Pretendard Variable\", -apple-system, BlinkMacSystemFont, system-ui, sans-serif' }}>
      {/* Hero Banner */}
          ...
          <div className="flex items-center h-full px-12">
            <div className="max-w-2xl space-y-6">
              <div className="inline-block">
                <Badge className="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-5 py-2.5 text-lg">
              ...
              <Button
                ...
                className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-lg px-10 py-7 h-auto"
              >
          ...
          {featuredGames.slice(0, 12).map((game) => (
            <Card key={game.id} className="bg-[#1a1f3a] border-purple-900/30 overflow-hidden group hover:scale-105 transition-transform cursor-pointer">
              <CardContent className="p-0">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                    <div className="absolute bottom-0 left-0 right-0 p-4">
                      <Button
                        ...
                        className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 h-10 text-base"
                      >
                ...
                <div className="p-3">
                  <p className="text-base font-medium truncate text-white">{game.name}</p>
                  <p className="text-sm text-gray-400 truncate">{game.game_providers?.name}</p>
                  <div className="flex items-center gap-1 mt-2">
                    <Badge variant="secondary" className="text-sm bg-green-900/30 text-green-400">
                      20000원
                    </Badge>
                    <span className="text-sm text-gray-500">X 2.56</span>
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((category) => (
            <Card 
              ...
              onClick={() => onRouteChange(`/benz/${category.id}`)}
            >
              <CardContent className="p-0">
              ...
              <table className="w-full">
                <thead className="border-b border-purple-900/30">
                  <tr className="text-gray-400 text-base">
                    <th className="text-left p-5">게임</th>
                    <th className="text-left p-5">유저</th>
                    <th className="text-left p-5">시간</th>
                    <th className="text-right p-5">배팅 금액</th>
                    <th className="text-right p-5">배수</th>
                    <th className="text-right p-5">지급금</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={index} className="border-b border-purple-900/10 hover:bg-purple-900/10">
                        <td className="p-5">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded bg-purple-900/30 flex items-center justify-center text-xl">
                              🎰
                            </div>
                            <span className="text-base text-white">{bet.game_title || 'Unknown Game'}</span>
                          </div>
                        </td>
                        <td className="p-5 text-base text-gray-300">{bet.username || 'gtgt***'}</td>
                        <td className="p-5 text-base text-gray-400">{formatDate(bet.played_at)}</td>
                        <td className="p-5 text-right">
                          <span className="text-base text-orange-400">🪙 {formatCurrency(bet.bet_amount || 0)}</span>
                        </td>
                        <td className="p-5 text-right text-base text-gray-300">{multiplier}x</td>
                        <td className="p-5 text-right">
                          <span className={`text-base font-medium ${profit > 0 ? 'text-green-400' : profit < 0 ? 'text-red-400' : 'text-gray-400'}`}>
